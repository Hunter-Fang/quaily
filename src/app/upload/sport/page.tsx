...
const PROMPT_TEMPLATE = `你是我的运动数据识别助手。我会发给你一张或多张运动记录截图，请识别图中数据，整理成以下 JSON 数组，直接输出 JSON，不要 markdown 代码块，不要任何解释。每张截图一条记录。

字段（严格按此字段名）：
- 活动名称: string（必填，如"泳池游泳 晨练"）
- 日期时间: string（必填，ISO 北京时间，如"2026-05-02T08:00:00.000+08:00"）
- 运动类型: string（必填，只能选：游泳 / 划船机 / 跑步 / 骑行 / 爬楼梯 / 自由训练 / 羽毛球）
- 总时长: string（必填，HH:MM:SS，如"01:31:46"）
- 消耗热量(kcal): number（必填）
...
- 恢复时间(小时): number
- 平均配速: string（只填数值，如"3'19"，不带单位）
- 心率区间分布: string（JSON字符串，如"{\\"zone1\\": 5, \\"zone2\\": 20, \\"zone3\\": 45, \\"zone4\\": 25, \\"zone5\\": 5}"）
- 备注: string

注意：图中没有的字段不输出，距离统一转为米，时长格式严格 HH:MM:SS。

示例：
[{"活动名称":"泳池游泳 晨练","日期时间":"2026-05-02T08:00:00.000+08:00","运动类型":"游泳","运动状态":"优秀","总时长":"01:31:46","总距离(m)":2000,"消耗热量(kcal)":450,"平均心率(bpm)":138,"最高心率(bpm)":162,"恢复时间(小时)":24,"有氧训练效果":3.2,"无氧训练效果":0.5,"平均配速":"3'19","心率区间分布":"{\\"zone1\\": 5, \\"zone2\\": 20, \\"zone3\\": 45, \\"zone4\\": 25, \\"zone5\\": 5}"}]
...
const FIELDS = [
  ...
  ["恢复时间(小时)",  "number",   "小时"],
  ["平均配速",        "string",   "只填数值，如: 3'19（不带单位）"],
  ["心率区间分布",    "string",   "JSON 字符串，zone1-zone5 各区间百分比"],
  ["备注",            "string",   "自由文本"],
];
