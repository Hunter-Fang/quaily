...
type Stats = NonNullable<Awaited<ReturnType<typeof getSportStats>>>;
...
export default function SportPage() {
  ...
  useEffect(() => {
    getSportStats().then((s) => { setStats(s); setLoading(false); }).catch(() => setLoading(false));
  }, []);

  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-32">
        <div className="w-8 h-8 border-2 rounded-full animate-spin" style={{ borderColor: "var(--c-brand)", borderTopColor: "transparent" }} />
      </div>
    );
  }
  ...
  if (!stats || stats.totalCount === 0) {
    return (
      <div className="kami-section-header text-center mt-10">
        <div className="eyebrow justify-center"><span />Sports</div>
        <h1>运动数据</h1>
  ...
  return (
    <div className="animate-fade-in-up">
      ...
      <section className="mb-14">
        ...
        {/* Bar chart for calories by type */}
            ...
            {sortedTypes.map(([type, data]) => {
              ...
              return (
                <div key={type} className="flex items-center gap-3">
                  <span className="w-16 text-xs truncate" style={{ color: "var(--c-text-3)" }}>
                    {ACTIVITY_EMOJI[type] || "🎯"} {type}
                  </span>
                  ...
                  <span className="w-16 text-right text-xs font-mono tabular-nums" style={{ color: "var(--c-text-3)" }}>
                    {data.cal}
                  </span>
                </div>
              );
        ...
        <div className="p-5 rounded-lg border grid gap-4 sm:grid-cols-6" style={{ background: "var(--c-surface)", borderColor: "var(--c-border)" }}>
          {months.map(([month, data]) => (
            <div key={month} className="text-center">
              <div className="text-[11px] mb-2" style={{ color: "var(--c-text-4)" }}>
                {month.slice(5)}
              </div>
              <div className="font-serif text-lg font-[500]" style={{ color: "var(--c-brand)" }}>
                {data.count}
              </div>
              ...
              <div className="w-full h-1 rounded mt-2" style={{ background: "var(--c-warm-sand)" }}>
                <div
                  ...
                  style={{
                    width: `${Math.min(100, (data.cal / maxMonthCal) * 100)}%`,
                    background: "var(--c-brand)",
                    opacity: 0.25,
                  }}
                />
            ...
            <table className="w-full text-sm">
              <thead>
                <tr style={{ borderBottom: "2px solid var(--c-brand)" }}>
                  ...
                  <th className="py-3 px-3 text-right font-serif font-normal" style={{ color: "var(--c-text-3)" }}>有氧/无氧</th>
                  <th className="py-3 px-3 text-right font-serif font-normal" style={{ color: "var(--c-text-3)" }}>恢复(h)</th>
                </tr>
              </thead>
              <tbody>
                {stats.hrRecords.map((r, i) => (
                  <tr
                    key={r.id}
                  ...
                  >
                    ...
                    <td className="py-2.5 px-3 text-right font-mono tabular-nums font-medium" style={{ color: "var(--c-brand)" }}>
                      {r.avgHR}
                    </td>
                    <td className="py-2.5 px-3 text-right font-mono tabular-nums font-medium" style={{ color: "var(--c-text-2)" }}>
                      {r.maxHR}
                    </td>
                    <td className="py-2.5 px-3 text-center">
                      <span className="inline-block px-2 py-0.5 rounded text-xs font-medium mr-1" style={{ background: "var(--c-tag-bg)", color: "var(--c-brand)" }}>
                        {r.aerobicEffect?.toFixed(1) || "-"}
                      </span>
                      <span className="inline-block px-2 py-0.5 rounded text-xs font-medium" style={{ background: "var(--c-tag-bg)", color: "var(--c-tag-border)" }}>
                        {r.anaerobicEffect?.toFixed(1) || "-"}
                      </span>
                    </td>
                    <td className="py-2.5 px-3 text-right font-mono tabular-nums" style={{ color: "var(--c-text-2)" }}>
                      {r.recoveryHours ?? "-"}
                    </td>
                  </tr>
                ))}
...
function ActivityRow({ record }: { record: any }) {
  return (
    <div
      className="post-card p-5 flex items-center gap-4 group cursor-pointer hover:opacity-90 transition-opacity"
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="font-medium text-sm truncate mb-0.5" style={{ color: "var(--c-text)" }}>
          {record.name}
        </div>
      ...
      <div className="text-right flex-shrink-0">
        <div className="font-serif text-base font-[500]" style={{ color: "var(--c-brand)" }}>
          {record.duration}
        </div>
        <div className="text-[11px] text-right mt-0.5" style={{ color: "var(--c-text-4)" }}>
